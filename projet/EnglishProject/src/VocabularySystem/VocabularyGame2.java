...
public class VocabularyGame2 {
	...
	public VocabularyGame2(TopicVocabulary selectedTopic, TypeOfGame typeOfGame, int numberOfWords, int numberOfPropositions, Logger logger) {
		if (selectedTopic != null && typeOfGame != null) {
			...
			for(Word w : wordsToPlay) {
				//generate a list of proposition for the word w and add the proposition w to have an access to the answer
				ArrayList<Word> wPropositions = generateRandomListOfWord(numberOfPropositions-1,this.selectedTopic,this.selectedTopic.getTopicVocabulary().indexOf(w));
	...
	public ArrayList<Word> generateRandomListOfWord(int numberOfWord, TopicVocabulary targetTopic, int avoidingIndex) {
		...
		//generate a list of random number between 0 and the number of word in the topic -1 which size is numberOfWord
		while (randomIndex.size() < numberOfWord) {
			int randomNumber = (int)(Math.random() * ((targetTopic.getTopicVocabulary().size()-1) + 1));
			if ( !(randomIndex.contains (randomNumber) ) && !(avoidingIndex == randomNumber) ) {
				randomIndex.add(randomNumber);
			}
		}
		//put into randomWord the word which correspond to the word at the index of randomIndex
	...
	public ArrayList<Integer> generateRandomListOfIndex(ArrayList<Word> words,String languageAsked) {
		...
		for (Word w : words) {
			...
			if (languageAsked.equals("ENGLISH")) {
				randomNumber = (int)(Math.random() * ((w.getEnglishWords().size()-1) + 1));
			}
			else if (languageAsked.equals("FRENCH")) {
				randomNumber = (int)(Math.random() * ((w.getFrenchWords().size()-1) + 1));
			}
			randomIndex.add(randomNumber);
		}
		return randomIndex;
	...
	public ArrayList<String> getLeftPropositions(int indexOfTheWord) {
		...
		for (int i=0 ; i < numberOfPropositions/2 ; i++) {
			if (this.type.equals(TypeOfGame.ENGLISH)) {
				leftPropositions.add(this.wordsPropositions.get(indexOfTheWord).get(i).getFrenchWords().get(this.propositionsIndex.get(indexOfTheWord).get(i)));
			}
			else if (this.type.equals(TypeOfGame.FRENCH)) {
				leftPropositions.add(this.wordsPropositions.get(indexOfTheWord).get(i).getEnglishWords().get(this.propositionsIndex.get(indexOfTheWord).get(i)));
			}
		}
	...
	public ArrayList<String> getRightPropositions(int indexOfTheWord) {
		...
		for (int i= (numberOfPropositions/2) ; i < numberOfPropositions ; i++) {
			if (this.type.equals(TypeOfGame.ENGLISH)) {
				rightPropositions.add(this.wordsPropositions.get(indexOfTheWord).get(i).getFrenchWords().get(this.propositionsIndex.get(indexOfTheWord).get(i)));
			}
			else if (this.type.equals(TypeOfGame.FRENCH)) {
				rightPropositions.add(this.wordsPropositions.get(indexOfTheWord).get(i).getEnglishWords().get(this.propositionsIndex.get(indexOfTheWord).get(i)));
			}
		}
